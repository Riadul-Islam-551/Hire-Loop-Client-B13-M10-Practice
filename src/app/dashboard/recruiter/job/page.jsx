...
const Page = async () => {
  ...
  const statusStyles = {
    active:
      "bg-success-50 text-success dark:bg-success-950/30 border-success-200/50 dark:border-success-800/30",
    inactive:
      "bg-default-100 text-default-600 dark:bg-default-800/50 border-default-200 dark:border-default-700/50",
    pending:
      "bg-warning-50 text-warning dark:bg-warning-950/30 border-warning-200/50 dark:border-warning-800/30",
  };
            ...
            <Table.ScrollContainer>
              <Table.Content className="min-w-200">
                <Table.Header>
                  <Table.Column
                    isRowHeader
                    className="bg-default-100/50 dark:bg-default-800/30 text-default-600 font-bold uppercase text-xs tracking-wider py-4 pl-6"
                  >
                    Job Title
                  </Table.Column>
                  ...
                  <Table.Column
                    align="center"
                    className="bg-default-100/50 dark:bg-default-800/30 text-default-600 font-bold uppercase text-xs tracking-wider py-4 pr-6 w-32"
                  >
                    Actions
                  </Table.Column>
                </Table.Header>
                ...
                <Table.Body
                  emptyContent={
                    "No job dynamic listings created yet for this profile."
                  }
                >
                  {jobs.map((job, ind) => {
                    const currentStatus = (
                    ...
                    return (
                      ...
                      >
                        ...
                        <Table.Cell className="py-4">
                          <span
                            className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold border capitalize tracking-wide ${statusStyles[currentStatus] || statusStyles.active}`}
                          >
